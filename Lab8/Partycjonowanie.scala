...
val fileName = "/FileStore/tables/Files/actors.csv"
...
//display(initialDF.orderBy("category"))
initialDF.count() // / 835513
...
// MAGIC %md
// MAGIC U mnie 835513 wierszy, to 835513/8=104 439 rekordów na partycje. 
...
val alternateDF = spark.read
  .format("parquet").load("/FileStore/tables/actors_p.parquet")

printf("Partitions: %d%n%n", alternateDF.rdd.getNumPartitions) //dwie partycje
...
alternateDF.repartition(8).count()//8 slotów 2 partycje
